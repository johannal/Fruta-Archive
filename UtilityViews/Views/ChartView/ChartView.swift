...
struct PieChartSliceView : View {
    ...
    private var labelPosition: CGPoint {
        ...
        let x = distanceFromCenter * CGFloat(cos(midAngle.radians))
        let y = distanceFromCenter * CGFloat(sin(midAngle.radians))

        return CGPoint(x: frame.midX + x, y: frame.midY + y)
    }
...
struct PieChart {
    ...
    init(labeledValues: [(label: String, value: Double)]) {
        ...
        var slices: [PieChartSlice] = []
        for i in 0..<labeledValues.count {
            let labeledValue = labeledValues[i]
            let ratio = labeledValue.value / totalValue
            let endAngle = angle + .degrees(360 * ratio)

            let hue = Double(i) / Double(labeledValues.count)
